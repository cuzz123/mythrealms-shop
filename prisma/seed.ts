...
async function main() {
  const seedAdminConfig = resolveSeedAdminConfig(process.env)
  ...
  const admin = seedAdminConfig
    ? await db.user.create({ data:{ name:'Admin', email:seedAdminConfig.email, password:await bcrypt.hash(seedAdminConfig.password,12), role:'ADMIN' }})
    : null
