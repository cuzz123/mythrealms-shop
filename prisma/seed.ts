...
async function main() {
  ...
  const cats = await Promise.all([
    ...
    db.category.create({ data:{ name:'Daily · 日常', slug:'daily', description:'Everyday jewelry with hidden mythical meaning. Minimalist design, ancient symbolism.', sortOrder:7 }}),
  ])
  const bc = cats[0]
  ...
  const prods = [
    ...
    { name:'Nine Moons Bracelet · 九月 — Sterling Silver', slug:'nine-moons-bracelet', description:'Nine tiny silver beads on a delicate chain. One for each tail of the Nine-Tailed Fox. No one will know what it means — except you.', details:JSON.stringify({specs:{itemType:'Beaded Bracelet',material:'925 Sterling Silver'}}), comparePrice:38.00, images:JSON.stringify([IMG.subtle_fox]), stone:'Silver',material:'925 Silver',intention:'Subtle Wisdom',isFeatured:true,categoryId:cats[6].id, variants:[{name:'16-18cm',price:28.99,stock:15}] },
    { name:'Azure Tides Bracelet · 青澜 — Blue Aventurine', slug:'azure-tides-bracelet', description:'Soft blue-grey aventurine beads. The color of the eastern sky at dawn. The Azure Dragon guards that sky — and now, your wrist.', details:JSON.stringify({specs:{itemType:'Beaded Bracelet',material:'Blue Aventurine & Silver'}}), comparePrice:42.00, images:JSON.stringify([IMG.subtle_dragon]), stone:'Aventurine',material:'Aventurine & Silver',intention:'Quiet Power',isFeatured:true,categoryId:cats[6].id, variants:[{name:'16-18cm',price:32.99,stock:12}] },
    { name:'Ember Pendant · 烬 — Gold Vermeil Feather', slug:'ember-pendant', description:'A single gold feather, no bigger than your fingernail. The Phoenix leaves a trace of fire wherever she goes. This is yours.', details:JSON.stringify({specs:{itemType:'Pendant Necklace',material:'Gold Vermeil'}}), comparePrice:55.00, images:JSON.stringify([IMG.subtle_phoenix]), stone:'Gold',material:'Gold Vermeil',intention:'Quiet Rebirth',isFeatured:true,categoryId:cats[6].id, variants:[{name:'40cm chain',price:42.99,stock:8}] },
    { name:'Gentle Hoof Bracelet · 仁蹄 — Jade & Bronze', slug:'gentle-hoof-bracelet', description:'Simple jade beads with one bronze accent. The Qilin refuses to step on living grass. You refuse to win at someone else's expense. This bracelet knows.', details:JSON.stringify({specs:{itemType:'Beaded Bracelet',material:'Jade & Bronze'}}), comparePrice:48.00, images:JSON.stringify([IMG.subtle_qilin]), stone:'Jade',material:'Jade & Bronze',intention:'Gentle Strength',isFeatured:true,categoryId:cats[6].id, variants:[{name:'16-18cm',price:36.99,stock:10}] },
    { name:'Horizon Ring · 天海 — Two-Tone Ceramic & Silver', slug:'horizon-ring', description:'Half ocean blue, half sky silver. The exact moment Kun becomes Peng. You're not who you were. You're not who you'll be. This ring marks the in-between.', details:JSON.stringify({specs:{itemType:'Stacking Ring',material:'Ceramic & 925 Silver'}}), comparePrice:52.00, images:JSON.stringify([IMG.subtle_kunpeng]), stone:'Ceramic',material:'Ceramic & Silver',intention:'Transformation',isFeatured:true,categoryId:cats[6].id, variants:[{name:'Size 6',price:38.99,stock:10},{name:'Size 7',price:38.99,stock:10}] },
    { name:'Frost Bracelet · 霜卫 — White Howlite', slug:'frost-bracelet', description:'White howlite with silver-grey veins. Like snow on a mountain peak. The White Tiger stands guard on that peak — silent, unseen, unthanked. Like you.', details:JSON.stringify({specs:{itemType:'Beaded Bracelet',material:'White Howlite & Silver'}}), comparePrice:35.00, images:JSON.stringify([IMG.subtle_tiger]), stone:'Howlite',material:'Howlite & Silver',intention:'Silent Guard',isFeatured:false,categoryId:cats[6].id, variants:[{name:'16-18cm',price:26.99,stock:18}] },
    { name:'Circle of Knowing Necklace · 知白 — Sterling Silver', slug:'circle-of-knowing-necklace', description:'A single silver circle. Empty. Open. Bai Ze knew 11,520 species and still chose to listen. Wisdom is not what you say. It's what you leave space for.', details:JSON.stringify({specs:{itemType:'Pendant Necklace',material:'925 Sterling Silver'}}), comparePrice:45.00, images:JSON.stringify([IMG.subtle_baize]), stone:'Silver',material:'925 Silver',intention:'Wisdom',isFeatured:false,categoryId:cats[6].id, variants:[{name:'45cm chain',price:34.99,stock:12}] },
    { name:'Star Map Studs · 星图 — Sterling Silver Earrings', slug:'star-map-studs', description:'Two tiny stars. One for each ear. The 28 Chinese mansions mapped the entire sky. Now you carry just two of them — enough to find your way home.', details:JSON.stringify({specs:{itemType:'Stud Earrings (Pair)',material:'925 Sterling Silver'}}), comparePrice:32.00, images:JSON.stringify([IMG.subtle_star]), stone:'Silver',material:'925 Silver',intention:'Celestial Guide',isFeatured:false,categoryId:cats[6].id, variants:[{name:'Standard',price:24.99,stock:20}] },
  ]
  ]
