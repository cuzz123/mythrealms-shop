...
type DraftEdits = Record<string, { title: string; description: string }>;
...
export function PinterestDraftQueue() {
  ...
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- initial internal review-queue hydration.
    void loadDrafts();
  }, [loadDrafts]);
  ...
  async function runAction(draft: Draft, action: "save" | "approve" | "reject") {
    ...
    try {
      ...
      applyDraft(data.draft);
      if (action === "approve") {
        setNotice("Internal candidate review 已通过；NO-GO，未提交且未发布。");
      }
    } catch (error) {
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-[var(--text-muted)]">
            Internal candidate review only · NO-GO · publishing is disabled · submitted = 0 · published = 0。
          </p>
        </div>
