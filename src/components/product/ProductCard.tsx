      ...
      <Link href={`/products/${product.slug}`} className="block">
        ...
        <div className="relative aspect-square overflow-hidden rounded-[var(--radius-lg)] bg-[var(--border-light)]">
          {isValidImage ? (
            <>
              {/* Primary image */}
              <Image src={imageUrl(image)} alt={`${product.name} — MythRealms intention bracelet`} fill sizes="(max-width: 640px) 50vw, (max-width: 1024px) 33vw, 25vw" loading="lazy"
              ...
              {hasSecondImage && (
                <Image src={imageUrl(secondImage)} alt={`${product.name} — alternate view, MythRealms crystal bracelet`} fill sizes="(max-width: 640px) 50vw, (max-width: 1024px) 33vw, 25vw" loading="lazy"
