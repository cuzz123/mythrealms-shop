    ...
    >
      {HOMEPAGE_HERO_SLIDES.map((slide, index) => (
        <Image
          ...
          preload={index === 0}
          sizes="100vw"
          className={`object-cover object-[58%_center] transition-opacity duration-700 sm:object-[65%_center] ${index === activeSlide ? "opacity-100" : "opacity-0"}`}
        />
      ))}
      ...
      <div className="relative mx-auto flex h-full max-w-7xl items-end px-6 pb-10 pt-24 sm:pb-16 sm:pt-28 lg:pb-20 lg:pt-32">
        <div className="max-w-xl">
          ...
          <div className="mt-8 flex gap-2" aria-label="Homepage editorial slides">
            {HOMEPAGE_HERO_SLIDES.map((slide, index) => (
              <button key={slide.src} type="button" onClick={() => setActiveSlide(index)} className={`h-1.5 w-7 transition-colors ${index === activeSlide ? "bg-white" : "bg-white/45 hover:bg-white/75"}`} aria-label={`Show ${slide.eyebrow}`} aria-current={index === activeSlide} />
            ))}
          </div>
        </div>
      </div>
