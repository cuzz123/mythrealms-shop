...
| Search Console sitemap + Google verification | sitemap 本地实现：动态读取已发布博客、公开商品/编辑/发现页，`revalidate=3600` 并去重；robots 指向 `/sitemap.xml`；根 metadata 有 Google verification token。Prisma client 恢复后，本地全量单测与 production build 已通过 | 历史只读 smoke 记录 `/sitemap.xml` HTTP 200；当前部署内容与 verification meta 未在本轮复核 | `待确认`：没有 Search Console 所有权确认、sitemap 提交/读取或抓取证据 | `待确认`：无 Coverage/Pages/Performance 导出 | `待确认`；HTTP 200 和 verification meta 不是收录基线 | `false`；需 property、完整 sitemap URL、提交/读取时间、状态、发现 URL、已索引 URL、页面/查询 clicks、impressions、CTR、position、时区与日期范围 |
...
本轮验证更新（2026-07-26）：主工作区 Prisma client 已恢复；`npm run test:unit` 为 `475/475` 通过，包含 SEO、结账和 Prisma 构建契约；`npm run build` 退出 `0` 并生成 166 个静态页面。浏览器 `release-surfaces` 为 `18/24` 通过，剩余 6 项为图片加载、旧商品数量/指南链接断言、首页样式与 hydration 状态问题，不能写成 E2E 全通过。未运行生产探针，未登录 GA4、Pinterest 或 Search Console。
...
- 2026-07-25：本地隔离 clean build 曾受不完整 `@prisma/client` 依赖树影响；后续生产部署已完成远端构建、TypeScript 与页面生成。
- 2026-07-26：已确认 `npm ci --ignore-scripts` 跳过 Prisma engine/client 的 postinstall，叠加多平台引擎冷下载缓慢，形成主工作区假性“生成挂起”。补齐同版本引擎后主工作区生成和 build 通过；仓库构建契约改为直接调用已安装的 Prisma CLI，并使用目标构建主机的 `native` 引擎。未执行生产部署。
