# ACT_WHITE_COAT_WALK_IN_STOP_01｜白色长外套·走入停下

- **状态：** candidate（候选待审）
- **质检：** pending_visual_review
- **角色：** `CHAR_HUNYUAN_WHITE_COAT_FULLBODY_001`
- **骨架：** `RIG_WHITE_COAT_FULLBODY`
- **配套镜头：** `CAM_WHITE_COAT_WALK_IN_STOP_01`（正面全身·落点定格）
- **规格：** 96 帧 @ 24 fps，共 4 秒

## 文件

- [动作预览](file:///D:/mythrealms-shop/video-pipeline/asset-library/06-motions/WHITE_COAT_WALK_PACK_001/preview/02_ACT_WHITE_COAT_WALK_IN_STOP_01.mp4)
- [动作中段帧](file:///D:/mythrealms-shop/video-pipeline/asset-library/06-motions/WHITE_COAT_WALK_PACK_001/preview/02_ACT_WHITE_COAT_WALK_IN_STOP_01_frames/frame_0048.png)
...
从动作包 Blend 的 `Action` 数据块追加 `ACT_WHITE_COAT_WALK_IN_STOP_01`，应用到 `RIG_WHITE_COAT_FULLBODY`。该动作使用 `root` 推进、左右 `foot_ik.*` 落脚和 `knee_pole.*` 膝盖朝向控制；不要直接移动高精度网格。
