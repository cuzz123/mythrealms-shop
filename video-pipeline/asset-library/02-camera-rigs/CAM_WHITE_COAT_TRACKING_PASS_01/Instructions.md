# CAM_WHITE_COAT_TRACKING_PASS_01｜右前三分侧·平行跟拍

- **状态：** candidate（候选待审）
- **质检：** pending_visual_review
- **配套动作：** `ACT_WHITE_COAT_TRACKING_PASS_01`（白色长外套·三分侧跟行）
- **角色：** `CHAR_HUNYUAN_WHITE_COAT_FULLBODY_001`
- **规格：** 96 帧 @ 24 fps，共 4 秒

## 文件

- [镜头预览](file:///D:/mythrealms-shop/video-pipeline/asset-library/06-motions/WHITE_COAT_WALK_PACK_001/preview/03_ACT_WHITE_COAT_TRACKING_PASS_01.mp4)
- [镜头中段帧](file:///D:/mythrealms-shop/video-pipeline/asset-library/06-motions/WHITE_COAT_WALK_PACK_001/preview/03_ACT_WHITE_COAT_TRACKING_PASS_01_frames/frame_0048.png)
...
从动作包 Blend 的 `Object` 数据块追加 `CAM_WHITE_COAT_TRACKING_PASS_01` 和对应 `FOCUS_*` 对焦物体，将场景相机切换到该 Camera。焦距保持在 55–75 mm，景深焦点绑定角色上身。
