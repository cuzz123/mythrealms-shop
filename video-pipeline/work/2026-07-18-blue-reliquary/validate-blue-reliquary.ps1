...
$shots = [regex]::Matches($text, '(?m)^## S(?:0[1-9]|1[0-5])A\r?$')
if ($shots.Count -ne 15) { throw "Expected 15 shot contracts, got $($shots.Count)" }
foreach ($id in 1..15) {
  $label = 'S{0:D2}A' -f $id
  $match = [regex]::Match($text, "(?s)^## $label`r?`n(.*?)(?=^## S(?:0[1-9]|1[0-5])A\r?$|^## Edit Contracts|\\z)", [System.Text.RegularExpressions.RegexOptions]::Multiline)
