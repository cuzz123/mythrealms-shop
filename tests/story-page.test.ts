...
test("About renders the approved Story and page-specific structured data", () => {
  ...
  assert.equal(aboutSchema.name, `Our Story | ${SITE_NAME}`);
...
test("Story crops the supplier mark out of the public product-reference frame", () => {
  const html = renderToStaticMarkup(createElement(AboutPage));

  assert.match(
    html,
    /<figure data-image-role="product-reference"[\s\S]*?class="[^"]*scale-\[1\.2\][^"]*"/,
  );
});
